...
import { AtlasCoords, ExpressionKey } from '@/lib/three/types';
import {
...
export class ExpressionBuffer {
    ...
    public setExpression(index: number, name: ExpressionKey) {
        ...
        if (!this.isBlinking[index]) this.setEyeOffset(index, config.eyes);
        if (!this.speakingStates[index]) this.setMouthOffset(index, config.mouth);
    }
    ...
    public update(delta: number) {
        ...
        if (needsUpdate) this.attribute.needsUpdate = true;
    }
}
